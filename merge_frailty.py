...
with open('frailty.html', 'r', encoding='utf-8') as f:
    old_html = f.read()
...
old_sections = re.findall(r'<section.*?</section>', old_html, re.DOTALL)
# Extract new sections from user input (provided in the prompt)
new_input = '''
        <section class="mb-16">
...
'''
new_sections = re.findall(r'<section.*?</section>', new_input, re.DOTALL)

# Sequence:
# 1. Old section 0 (Intro & Paradigm Shift)
# 2. New section 0 (Fried)
# 3. New section 1 (CFS)
# 4. Old section 1 (CGA)
# 5. New section 2 (SARC-F & eFI)
# 6. Old section 2 (Less is more)
# 7. New section 3 (Triage)
# 8. Old section 3 (Enablers)
...
# Extract the old script block content
old_script_match = re.search(r'<script>(.*)</script>', old_html, re.DOTALL)
old_script = old_script_match.group(1) if old_script_match else ""

# The old script contains its own wrapLabel, tooltips, and Chart defaults. We can remove them from old_script since they are duplicated or slightly different, and use the new global ones, but to be safe, I'll extract just the chart instantiations from the old script.
chart_instantiations = []
for var_name in ['ctxTrajectory', 'ctxSystemShift', 'ctxCGA', 'ctxLessIsMore']:
    pattern = rf'const {var_name} = document.getElementById.*?new Chart\([^;]+;'
    match = re.search(pattern, old_script, re.DOTALL)
    if match:
        chart_instantiations.append(match.group(0))

new_html += "\\n    <script>\\n        document.addEventListener(\\"DOMContentLoaded\\", function() {\\n"
new_html += new_script_input
new_html += "\\n" + "\\n\\n".join(chart_instantiations)
new_html += "\\n        });\\n    </script>\\n</body>\\n</html>"

with open('d:/antigravity/פרויקט בקרות/frailty.html', 'w', encoding='utf-8') as f:
    f.write(new_html)
