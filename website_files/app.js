...
window.addProjectToGallery = function(input) {
    ...
    Array.from(input.files).forEach(file => {
        ...
        reader.onload = function(e) {
            ...
            const projectName = prompt('הזן את שם הפרויקט החדש:', file.name.split('.')[0]) || file.name;
            const projectOwner = prompt('מי אחראי על הפרויקט?', 'רכז פרויקט') || 'רכז פרויקט';

            div.innerHTML = `
                ${previewHtml}
                <h4 style="color: #e2e8f0; margin-bottom: 5px;">${projectName}</h4>
                <p style="font-size:0.9rem; color: #cbd5e1; margin-bottom: 15px;">אחראי: ${projectOwner}</p>
                <button class="btn btn-sm btn-outline" style="margin-top: auto; margin-bottom: 0;" onclick="alert('פותח את תצוגת הפרויקט המלאה')">הצג פרויקט מלא</button>
                <button class="btn btn-sm" style="background: rgba(239, 68, 68, 0.2); color: #ef4444; border: none; position: absolute; top: 10px; left: 10px; padding: 5px; border-radius: 50%;" onclick="this.parentElement.remove()" title="מחק פרויקט">
